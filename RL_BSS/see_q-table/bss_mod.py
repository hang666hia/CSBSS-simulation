...
class BSSEnvironment:
    ...
    def __init__(self, max_steps, num_of_charging_slots, ini_full_battery, arrival_rate):
        self.t = 0
        ...
        self.n_features = 3 + self.num_of_charging_slots
        # 设置各种奖励的权重w
    ...
    def action_valid(self, action):
        ...
        if my_fun.count_nonzero(action_vector) > self.depleted_battery:
            valid = False
        else:
            for i in range(self.num_of_charging_slots):
    ...
    def put_battery_to_pile(self, action):
        ...
        for i in range(self.num_of_charging_slots):
            self.e_price_reward += (1 - e_price) * abs(self.charging_slots[i])
    ...
    def get_system_state(self):
        s1 = [self.t, self.depleted_battery, self.full_battery]
